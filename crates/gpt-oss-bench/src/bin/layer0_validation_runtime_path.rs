...
#[cfg(feature = "cuda")]
use gpt_oss_gpu::{cublas::CublasHandle, cublaslt_ops::CublasLtOps, CudaContext};
#[cfg(feature = "cuda")]
...
struct Cli {
    ...
    /// Fused-linear/addmm backend-discriminator candidate matrix status.
    #[arg(
        long,
        default_value = "/tmp/fused_linear_addmm_backend_discriminator_candidate_status.json"
    )]
    fused_linear_addmm_backend_discriminator_candidate_status: PathBuf,
...
fn main() -> Result<()> {
    ...
    match cli.mode {
        ...
        Mode::FusedLinearAddmmHelperCandidate => run_fused_linear_addmm_helper_candidate(&cli),
...
fn run_fused_linear_addmm_helper_candidate(cli: &Cli) -> Result<()> {
    ...
    anyhow::ensure!(
        requested_layers == required_layers,
        "fused-linear/addmm helper candidate requires layers 6,10,13,16,18,21; got {:?}",
        requested_layers
    );

    let required_statuses = [
        (
            "backend_discriminator_candidate_matrix",
            &cli.fused_linear_addmm_backend_discriminator_candidate_status,
        ),
        ("producer_api_13_16_10", &cli.producer_api_status_13_16_10),
        ("producer_api_18_21", &cli.producer_api_status_18_21),
        ("layer6_api_probe", &cli.layer6_api_probe_status),
    ...
    let previous_candidate_status = if missing_required_statuses.is_empty() {
        load_json(&cli.fused_linear_addmm_backend_discriminator_candidate_status)?
    } else {
        Value::Null
    };
    ...
    if missing_required_statuses.is_empty() {
        ...
        for layer in requested_layers.iter().copied() {
            match evaluate_fused_linear_addmm_helper_layer_candidates(
                layer,
                model,
                cli,
    ...
    let candidate_matrix =
        build_fused_linear_addmm_helper_candidate_matrix(&layers_evaluated, &requested_layers);
    let full_sampled_set_clear = candidate_matrix.iter().any(|candidate| {
        candidate
            .get("candidate_full_vector_cleared_sampled_set")
            .and_then(Value::as_bool)
    ...
    let status = json!({
        ...
        "source_statuses": {
            "backend_discriminator_candidate_matrix": cli.fused_linear_addmm_backend_discriminator_candidate_status.display().to_string(),
            "producer_api_13_16_10": cli.producer_api_status_13_16_10.display().to_string(),
            "producer_api_18_21": cli.producer_api_status_18_21.display().to_string(),
            "layer6_api_probe": cli.layer6_api_probe_status.display().to_string(),
            "layer6_backend_discriminator": cli.layer6_backend_discriminator_status.display().to_string(),
        },
        "source_classification": previous_candidate_status
            .get("classification")
        ...
        "layers_evaluated": layers_evaluated,
        "layers_blocked": layers_blocked,
        "candidate_helpers": fused_linear_addmm_helper_candidates()
            .into_iter()
            .map(|candidate| candidate.status_row())
            .collect::<Vec<_>>(),
        "candidate_matrix": candidate_matrix,
        ...
        "decision_rules": {
            "full_sampled_set_exactness_required": true,
            "full_vector_required": true,
            "zero_mismatches_required": true,
            "max_abs_diff_zero_required": true,
            "focus_lane_only_selection_allowed": false,
            ...
            "diagnostic_candidates_selectable": false,
            "evidence_only_candidates_selectable": false,
            "unavailable_helpers_are_failures": false,
            "backend_selection_authorized": false,
        },
        "selected_backend": Value::Null,
        "backend_selected": false,
        ...
        "server_claim": false,
        "context_length_claim": false,
        "next_bounded_step": "Review helper candidate matrix before any backend selection, consumer revalidation, or promotion proof plan.",
    });
    write_json(&cli.output, &status)
}

fn run_fused_linear_addmm_like_helper_prototype(cli: &Cli) -> Result<()> {
    let requested_layers = if let Some(layers) = cli.layers.as_deref() {
        parse_comma_or_range_layers(layers)?
    } else {
        vec![6, 10, 13, 16, 18, 21]
    ...
    anyhow::ensure!(
        requested_layers == required_layers,
        "fused-linear/addmm-like helper prototype requires layers 6,10,13,16,18,21; got {:?}",
        requested_layers
    );

    let helper_candidate_status =
        PathBuf::from("/tmp/fused_linear_addmm_helper_candidate_status.json");
    let required_statuses = [
        ("helper_candidate_status", &helper_candidate_status),
        (
            "backend_discriminator_candidate_matrix",
            &cli.fused_linear_addmm_backend_discriminator_candidate_status,
        ),
    ...
    let helper_candidate_source = if missing_required_statuses.is_empty() {
        load_json(&helper_candidate_status)?
    } else {
        Value::Null
    };
    ...
    if missing_required_statuses.is_empty() {
        ...
        for layer in requested_layers.iter().copied() {
            match evaluate_fused_linear_addmm_like_helper_layer(layer, model, cli) {
                Ok(layer_status) => layers_evaluated.push(layer_status),
                Err(err) => layers_blocked.push(json!({
                    "layer_index": layer,
                    "reason": err.to_string(),
    ...
    let full_sampled_set_clear = layers_evaluated.len() == requested_layers.len()
        && layers_blocked.is_empty()
        && layers_evaluated.iter().all(|layer| {
            layer
                .get("candidate_result")
                .and_then(|candidate| candidate.get("full_vector_cleared"))
                .and_then(Value::as_bool)
                .unwrap_or(false)
                && layer
                    .get("candidate_result")
                    .and_then(|candidate| candidate.get("max_abs_diff"))
                    .and_then(Value::as_f64)
                    .is_some_and(|max_abs_diff| max_abs_diff == 0.0)
        });
    let candidate_executed_any = layers_evaluated.iter().any(|layer| {
        layer
            .get("candidate_result")
            .and_then(|candidate| candidate.get("candidate_executed"))
            .and_then(Value::as_bool)
            .unwrap_or(false)
    });
    let classification = if !missing_required_statuses.is_empty() {
        "fused_linear_addmm_like_helper_candidate_blocked_by_missing_reference"
    } else if !candidate_executed_any {
        "fused_linear_addmm_like_helper_candidate_blocked_by_missing_cublaslt_api"
    } else if full_sampled_set_clear {
        "fused_linear_addmm_like_helper_candidate_full_sampled_set_clear"
    } else if layers_blocked.is_empty() && layers_evaluated.len() == requested_layers.len() {
        "fused_linear_addmm_like_helper_candidate_no_candidate_selected"
    } else {
        "fused_linear_addmm_like_helper_candidate_execution_failed"
    };
    let total_mismatches = layers_evaluated
        .iter()
        .filter_map(|layer| {
            layer
                .get("candidate_result")
                .and_then(|candidate| candidate.get("full_vector_mismatches"))
                .and_then(Value::as_u64)
        })
        .sum::<u64>();
    ...
    let status = json!({
        "classification": classification,
        "validation_only": true,
        "candidate_execution": candidate_executed_any,
        "runtime_behavior_changed": false,
        "production_routing_changed": false,
        "cuda_kernels_changed": false,
        "operator": "attention_o_proj",
        "candidate": "cublaslt_bf16_matmul_bias_epilogue",
        "reference": {
            "api": "module/F.linear/_C/addmm",
            "dtype": "torch.bfloat16",
            "fused_bias": true,
        ...
        "cublaslt_api": {
            "repo_wrapper_present": true,
            "toolchain_header_hint": "/usr/include/cublasLt.h",
            "bench_cuda_feature_enables_gpt_oss_gpu_cublaslt": true,
            "helper": "CublasLtOps::bf16_matmul_bias_epilogue_a_bt",
        },
        "source_statuses": {
            "helper_candidate_status": helper_candidate_status.display().to_string(),
            "backend_discriminator_candidate_matrix": cli.fused_linear_addmm_backend_discriminator_candidate_status.display().to_string(),
            "producer_api_13_16_10": cli.producer_api_status_13_16_10.display().to_string(),
            "producer_api_18_21": cli.producer_api_status_18_21.display().to_string(),
            "layer6_api_probe": cli.layer6_api_probe_status.display().to_string(),
        ...
        "source_classification": helper_candidate_source
            .get("classification")
            .and_then(Value::as_str)
            .unwrap_or("missing"),
        "missing_required_statuses": missing_required_statuses,
        ...
        "candidate_matrix": [{
            "candidate": "cublaslt_bf16_matmul_bias_epilogue",
            "layers": prototype_layer_matrix(&layers_evaluated),
            "total_mismatches": total_mismatches,
            "candidate_full_vector_cleared_sampled_set": full_sampled_set_clear,
            "candidate_selected": false,
            "classification": if full_sampled_set_clear {
        ...
        "decision_rules": {
            ...
            "evidence_only_candidates_selectable": false,
            "backend_selection_authorized": false,
            "separate_promotion_proof_required": true,
        },
        "selected_backend": Value::Null,
        "backend_selected": false,
        "implementation_authorized": false,
        ...
        "server_claim": false,
        "context_length_claim": false,
        "next_bounded_step": if full_sampled_set_clear {
            "Review a separate promotion proof plan before any backend selection, consumer revalidation, or runtime discussion."
        } else {
            "Review cuBLASLt fused-bias prototype mismatches before any additional helper design."
        },
    });
    write_json(&cli.output, &status)
}
...
#[derive(Clone, Copy)]
enum FusedLinearAddmmHelperCandidateKind {
    Local(OprojPolicy),
    CublasTensorOp,
    CublasPedantic,
    Unavailable,
...
#[derive(Clone, Copy)]
struct FusedLinearAddmmHelperCandidate {
    family: &'static str,
    kind: FusedLinearAddmmHelperCandidateKind,
    diagnostic_only: bool,
    evidence_only: bool,
}

impl FusedLinearAddmmHelperCandidate {
    fn available(self) -> bool {
        !matches!(self.kind, FusedLinearAddmmHelperCandidateKind::Unavailable)
    }

    fn policy(self) -> Option<&'static str> {
        match self.kind {
            FusedLinearAddmmHelperCandidateKind::Local(policy) => Some(policy.name()),
            FusedLinearAddmmHelperCandidateKind::CublasTensorOp => Some("cublas_bf16_tensor_op"),
            FusedLinearAddmmHelperCandidateKind::CublasPedantic => {
                Some("cublas_bf16_pedantic_or_deterministic")
            }
            FusedLinearAddmmHelperCandidateKind::Unavailable => None,
        }
    }
    ...
    fn status_row(self) -> Value {
        json!({
            ...
            "reason": if self.available() {
                ...
                "candidate_helper_unavailable_at_required_validation_abstraction"
            },
        })
    }
}

fn fused_linear_addmm_helper_candidates() -> Vec<FusedLinearAddmmHelperCandidate> {
    vec![
        FusedLinearAddmmHelperCandidate {
            family: "current_sequential_f32_bf16_output",
            kind: FusedLinearAddmmHelperCandidateKind::Local(OprojPolicy::Current),
            diagnostic_only: false,
            evidence_only: false,
        },
        FusedLinearAddmmHelperCandidate {
            family: "reverse_f32_bf16_output",
            kind: FusedLinearAddmmHelperCandidateKind::Local(OprojPolicy::Reverse),
            diagnostic_only: false,
            evidence_only: false,
        },
        FusedLinearAddmmHelperCandidate {
            family: "pairwise_f32_bf16_output",
            kind: FusedLinearAddmmHelperCandidateKind::Local(OprojPolicy::Pairwise),
            diagnostic_only: false,
            evidence_only: false,
        },
        FusedLinearAddmmHelperCandidate {
            family: "chunked_pairwise_f32_bf16_output",
            kind: FusedLinearAddmmHelperCandidateKind::Local(OprojPolicy::ChunkedPairwise),
            diagnostic_only: false,
            evidence_only: false,
        },
        FusedLinearAddmmHelperCandidate {
            family: "f64_diagnostic",
            kind: FusedLinearAddmmHelperCandidateKind::Local(OprojPolicy::F64Diagnostic),
            diagnostic_only: true,
            evidence_only: false,
        },
        FusedLinearAddmmHelperCandidate {
            family: "bf16_prebias_evidence_guard",
            kind: FusedLinearAddmmHelperCandidateKind::Local(OprojPolicy::PreBiasRound),
            diagnostic_only: false,
            evidence_only: true,
        },
        FusedLinearAddmmHelperCandidate {
            family: "bf16_product_evidence_guard",
            kind: FusedLinearAddmmHelperCandidateKind::Unavailable,
            diagnostic_only: false,
            evidence_only: true,
        },
        FusedLinearAddmmHelperCandidate {
            family: "cublas_bf16_tensor_op",
            kind: FusedLinearAddmmHelperCandidateKind::CublasTensorOp,
            diagnostic_only: false,
            evidence_only: false,
        },
        FusedLinearAddmmHelperCandidate {
            family: "cublas_bf16_pedantic_or_deterministic",
            kind: FusedLinearAddmmHelperCandidateKind::CublasPedantic,
            diagnostic_only: false,
            evidence_only: false,
        },
    ]
...
fn evaluate_fused_linear_addmm_helper_layer_candidates(
    layer: usize,
    model: &Path,
    cli: &Cli,
    previous_candidate_status: &Value,
) -> Result<Value> {
    let hidden = 2880usize;
    let q_dim = cli.query_heads * cli.head_dim;
    let role = fused_linear_addmm_layer_role(layer);
    ...
    let previous_layer = previous_candidate_status
    ...
    let mut candidate_results = Vec::new();
    for candidate in fused_linear_addmm_helper_candidates() {
        let result = match candidate.kind {
            FusedLinearAddmmHelperCandidateKind::Local(policy) => {
                ...
                fused_linear_addmm_helper_candidate_result(
                    candidate,
                    Some(policy.name()),
                    &output,
                    &oproj_reference,
            ...
            FusedLinearAddmmHelperCandidateKind::CublasTensorOp => {
                match compute_attention_oproj_cublas_bf16(
                    &weighted_v,
                    &oproj_weight,
                    &oproj_bias,
                    false,
                ) {
                    Ok((output, metadata)) => fused_linear_addmm_helper_candidate_result(
                        candidate,
                        candidate.policy(),
                        &output,
                        &oproj_reference,
                    ...
                    Err(err) => fused_linear_addmm_unavailable_helper_result(
                        candidate,
                        focus_lane,
                        oproj_reference[focus_lane],
                        &attention_status_path,
                        cli,
                        format!("cublas_tensor_op_unavailable: {err}"),
                    ),
                }
            }
            FusedLinearAddmmHelperCandidateKind::CublasPedantic => {
                match compute_attention_oproj_cublas_bf16(
                    ...
                    Err(err) => fused_linear_addmm_unavailable_helper_result(
                        ...
                        &attention_status_path,
                        cli,
                        format!("cublas_pedantic_unavailable: {err}"),
                    ),
                }
            }
            FusedLinearAddmmHelperCandidateKind::Unavailable => {
                fused_linear_addmm_unavailable_helper_result(
                    candidate,
                    focus_lane,
                    oproj_reference[focus_lane],
                    &attention_status_path,
                    cli,
                    "candidate_helper_unavailable_at_required_validation_abstraction".to_string(),
                )
            }
        };
        candidate_results.push(result);
    ...
    Ok(json!({
        ...
        "role": role,
        "focus_lane": focus_lane,
        "producer_api_reference_available": previous_layer
            .get("producer_api_reference_available")
            .and_then(Value::as_bool)
            .unwrap_or(false),
        "producer_api_full_vector_clear": previous_layer
            .get("producer_api_full_vector_clear")
            .and_then(Value::as_bool)
            .unwrap_or(false),
        "previous_candidate_status": {
            "classification": previous_candidate_status
                .get("classification")
                .and_then(Value::as_str)
                .unwrap_or("missing"),
            "path": cli.fused_linear_addmm_backend_discriminator_candidate_status.display().to_string(),
        },
        "reference": {
            "api": "module/F.linear/_C/addmm",
            "dtype": "torch.bfloat16",
...
fn evaluate_fused_linear_addmm_like_helper_layer(
    layer: usize,
    model: &Path,
    cli: &Cli,
) -> Result<Value> {
    ...
    let candidate_result = match compute_attention_oproj_cublaslt_bf16_bias_epilogue(
        &weighted_v,
        &oproj_weight,
        &oproj_bias,
    ) {
        Ok((output, metadata)) => fused_linear_addmm_like_helper_candidate_result(
            &output,
            &oproj_reference,
            focus_lane,
            &attention_status_path,
        ...
        Err(err) => fused_linear_addmm_like_helper_unavailable_result(
            focus_lane,
            oproj_reference[focus_lane],
            &attention_status_path,
            format!("cublaslt_bf16_matmul_bias_epilogue_unavailable: {err}"),
        ),
    };
    ...
    Ok(json!({
        ...
        "tensor_metadata": {
            "input": tensor_layout_metadata("weighted_v", "torch.bfloat16", &[1, q_dim], &[q_dim, 1]),
            "weight": tensor_layout_metadata("o_proj_weight", "torch.bfloat16", &[hidden, q_dim], &[q_dim, 1]),
            "bias": tensor_layout_metadata("o_proj_bias", "torch.bfloat16", &[hidden], &[1]),
            "output": tensor_layout_metadata("o_proj_output", "torch.bfloat16", &[1, hidden], &[hidden, 1]),
        },
        "model_tensors": {
            "oproj_weight": oproj_weight_source,
            "oproj_bias": oproj_bias_source,
...
fn tensor_layout_metadata(name: &str, dtype: &str, shape: &[usize], stride: &[usize]) -> Value {
    json!({
        "name": name,
        "dtype": dtype,
        "shape": shape,
        "stride": stride,
        "contiguous": true,
        "layout": "row_major_original_artifact_order",
    })
}

fn fused_linear_addmm_helper_candidate_result(
    candidate: FusedLinearAddmmHelperCandidate,
    policy: Option<&str>,
    output: &[f32],
    reference: &[f32],
    focus_lane: usize,
    ...
    cli: &Cli,
    helper_metadata: Value,
) -> Value {
    let metric = compare_hidden(output, reference);
    let focus_lane_local = output[focus_lane];
    let focus_lane_reference = reference[focus_lane];
    let focus_lane_abs_diff = (focus_lane_local - focus_lane_reference).abs();
    let full_vector_cleared = metric.metrics.mismatches == 0 && metric.metrics.max_abs_diff == 0.0;
    let focus_lane_cleared = focus_lane_abs_diff == 0.0;
    let selectable = full_vector_cleared && !candidate.diagnostic_only && !candidate.evidence_only;
    let rejection_reason = if selectable {
        Value::Null
    } else if candidate.diagnostic_only {
        json!("diagnostic_only")
...
fn fused_linear_addmm_like_helper_candidate_result(
    output: &[f32],
    reference: &[f32],
    focus_lane: usize,
    attention_status_path: &Path,
    ...
    json!({
        ...
        "candidate_available": true,
        "candidate_executed": true,
        "selectable": full_vector_cleared,
        "rejection_reason": if full_vector_cleared {
            Value::Null
        } else {
            json!("full_vector_mismatches")
        },
...
fn fused_linear_addmm_like_helper_unavailable_result(
    focus_lane: usize,
    focus_lane_reference: f32,
    attention_status_path: &Path,
    reason: String,
) -> Value {
    json!({
...
fn fused_linear_addmm_unavailable_helper_result(
    candidate: FusedLinearAddmmHelperCandidate,
    focus_lane: usize,
    focus_lane_reference: f32,
    attention_status_path: &Path,
...
fn prototype_layer_matrix(layers: &[Value]) -> Value {
    let mut layer_map = serde_json::Map::new();
    for layer in layers {
        let layer_index = layer
            ...
            .and_then(Value::as_u64)
            .unwrap_or_default();
        let candidate = layer.get("candidate_result").unwrap_or(&Value::Null);
        layer_map.insert(
            layer_index.to_string(),
            json!({
                "full_vector_cleared": candidate
                    .get("full_vector_cleared")
                    .and_then(Value::as_bool)
                    .unwrap_or(false),
                ...
                "candidate_available": candidate
                    ...
                    .and_then(Value::as_bool)
                    .unwrap_or(false),
                "candidate_executed": candidate
                    .get("candidate_executed")
                    .and_then(Value::as_bool)
                    .unwrap_or(false),
            }),
        );
...
fn build_fused_linear_addmm_helper_candidate_matrix(
    layers: &[Value],
    requested_layers: &[usize],
) -> Vec<Value> {
    fused_linear_addmm_helper_candidates()
        .into_iter()
        .map(|candidate| {
            let mut layer_map = serde_json::Map::new();
            let mut blocked_family_layers_cleared = Vec::new();
            ...
            for layer_status in layers {
                ...
                let selectable = candidate_result
                    ...
                    .and_then(Value::as_bool)
                    .unwrap_or(false);
                let available = candidate_result
                    .and_then(|result| result.get("candidate_available"))
                    .and_then(Value::as_bool)
                    .unwrap_or(false);
                if !selectable {
                    selectable_all_layers = false;
                ...
                if !available {
                    unavailable_layers.push(layer_index);
                }
                total_mismatches += mismatches.unwrap_or(1_000_000);
                if matches!(role, "blocked_family" | "raw_qk_solved_oproj_blocked")
                    && full_vector_cleared
            ...
            let classification = if !candidate.available() || !unavailable_layers.is_empty() {
                "candidate_unavailable"
            } else if candidate.diagnostic_only {
                "candidate_diagnostic_only"
            } else if candidate.evidence_only {
...
fn run_raw_qk_single_mismatch_debug(cli: &Cli) -> Result<()> {
    let layer = cli.layer_index;
    let q_head = cli.q_head;
    let key_column = cli.key_column;
    let head_dim = cli.head_dim;
    let q_dim = cli.query_heads * head_dim;
    let kv_dim = cli.kv_heads * head_dim;
    let k_history_count = cli.token_count * kv_dim;
    let raw_count = cli.query_heads * cli.token_count;
    let masked_count = cli.query_heads * (cli.token_count + 1);
    anyhow::ensure!(
        q_head < cli.query_heads,
        "q-head {q_head} out of range for {} query heads",
        cli.query_heads
    );
    anyhow::ensure!(
        key_column < cli.token_count,
        "key-column {key_column} must be a real-token column < {}",
        cli.token_count
    );
    let kv_head = q_head / cli.heads_per_kv;
    anyhow::ensure!(
        kv_head < cli.kv_heads,
        "mapped kv_head {kv_head} out of range for {} kv heads",
        cli.kv_heads
    );
    ...
    let (probs_status, _probs_oracle) =
    ...
    let raw_index = q_head * cli.token_count + key_column;
    let masked_index = q_head * (cli.token_count + 1) + key_column;
    let q_base = q_head * head_dim;
    let k_base = (key_column * cli.kv_heads + kv_head) * head_dim;
    let (q_vector, k_vector, terms) = if schema_blocked {
        (&[][..], &[][..], Vec::new())
    } else {
        let q_vector = &q_post[q_base..q_base + head_dim];
        let k_vector = &k_post[k_base..k_base + head_dim];
        let terms = q_vector
            .iter()
            .zip(k_vector.iter())
            .map(|(&q, &k)| q * k)
            .collect::<Vec<_>>();
        (q_vector, k_vector, terms)
    };

    let official_value = raw_oracle.get(raw_index).copied().unwrap_or(f32::NAN);
    let official_masked_value = masked_oracle.get(masked_index).copied().unwrap_or(f32::NAN);
    let make_variant = |name: &str, policy: &str, unscaled_sum: f64, scaled: f64, output: f32| {
        json!({
            "name": name,
            "policy": policy,
            "unscaled_sum": unscaled_sum,
            "scaled_before_output_round": scaled,
            "output": output,
            "abs_diff": (output - official_value).abs(),
            "matches_official": output == official_value,
        })
    };
    let mut variants = Vec::new();
    if !schema_blocked {
        let sequential_sum = terms.iter().fold(0.0f32, |sum, value| sum + *value);
        let sequential_scaled = sequential_sum * cli.scale;
        variants.push(make_variant(
            "current_local_policy",
            "sequential f32 dot, scale after sum, BF16 output",
            sequential_sum as f64,
            sequential_scaled as f64,
            round_bf16(sequential_scaled),
        ));

        let scale_per_term_sum = terms
            .iter()
            .fold(0.0f32, |sum, value| sum + (*value * cli.scale));
        variants.push(make_variant(
            "sequential_f32_scale_per_term_bf16_output",
            "sequential f32 dot with scale applied per term, BF16 output",
            (scale_per_term_sum / cli.scale) as f64,
            scale_per_term_sum as f64,
            round_bf16(scale_per_term_sum),
        ));

        let reverse_sum = terms.iter().rev().fold(0.0f32, |sum, value| sum + *value);
        variants.push(make_variant(
            "reverse_f32_sum_scale_after_bf16_output",
            "reverse f32 dot, scale after sum, BF16 output",
            reverse_sum as f64,
            (reverse_sum * cli.scale) as f64,
            round_bf16(reverse_sum * cli.scale),
        ));

        let pairwise_sum = pairwise_sum_f32(&terms);
        variants.push(make_variant(
            "pairwise_f32_sum_scale_after_bf16_output",
            "pairwise f32 dot, scale after sum, BF16 output",
            pairwise_sum as f64,
            (pairwise_sum * cli.scale) as f64,
            round_bf16(pairwise_sum * cli.scale),
        ));

        let f64_sum = terms.iter().fold(0.0f64, |sum, value| sum + *value as f64);
        variants.push(make_variant(
            "f64_sum_scale_after_bf16_output",
            "f64 diagnostic dot, scale after sum, BF16 output",
            f64_sum,
            f64_sum * cli.scale as f64,
            round_bf16((f64_sum * cli.scale as f64) as f32),
        ));

        let mut abs_terms = terms.clone();
        abs_terms.sort_by(|left, right| left.abs().total_cmp(&right.abs()));
        let abs_sum = abs_terms.iter().fold(0.0f32, |sum, value| sum + *value);
        variants.push(make_variant(
            "deterministic_abs_ascending_f32_sum_scale_after_bf16_output",
            "deterministic f32 dot sorted by ascending absolute product magnitude",
            abs_sum as f64,
            (abs_sum * cli.scale) as f64,
            round_bf16(abs_sum * cli.scale),
        ));

        let bf16_product_sum = terms
            .iter()
            .fold(0.0f32, |sum, value| sum + round_bf16(*value));
        variants.push(make_variant(
            "bf16_product_then_f32_sum_scale_after_bf16_output",
            "evidence-only f32 dot after each product is rounded to BF16",
            bf16_product_sum as f64,
            (bf16_product_sum * cli.scale) as f64,
            round_bf16(bf16_product_sum * cli.scale),
        ));

        variants.push(make_variant(
            "sequential_f32_scale_after_no_output_round",
            "sequential f32 dot, scale after sum, no BF16 output round",
            sequential_sum as f64,
            sequential_scaled as f64,
            sequential_scaled,
        ));
    ...
    let current_local_value = variants
        .iter()
        .find(|entry| entry.get("name").and_then(Value::as_str) == Some("current_local_policy"))
        .and_then(|entry| entry.get("output"))
        .and_then(Value::as_f64)
        .map(|value| value as f32)
        .unwrap_or(f32::NAN);
    let abs_diff = (current_local_value - official_value).abs();

    let mut gqa_variants = Vec::new();
    if !schema_blocked {
        for candidate_kv_head in 0..cli.kv_heads {
            let candidate_k_base = (key_column * cli.kv_heads + candidate_kv_head) * head_dim;
            let candidate_k = &k_post[candidate_k_base..candidate_k_base + head_dim];
            let sum = q_vector
                .iter()
                .zip(candidate_k.iter())
                .fold(0.0f32, |sum, (&q, &k)| sum + q * k);
            let output = round_bf16(sum * cli.scale);
            gqa_variants.push(json!({
                "kv_head": candidate_kv_head,
                "expected_mapping": candidate_kv_head == kv_head,
                "output": output,
                "matches_official": output == official_value,
                "abs_diff": (output - official_value).abs(),
            }));
    ...
    let official_bf16 = bf16::from_f32(official_value);
    let official_bits = official_bf16.to_bits();
    let prev_bf16 = if official_bits > 0 {
        Some(bf16::from_bits(official_bits - 1).to_f32())
    } else {
        None
    };
    let next_bf16 = Some(bf16::from_bits(official_bits + 1).to_f32());
    let one_ulp_like = prev_bf16 == Some(current_local_value)
        || next_bf16 == Some(current_local_value)
        || prev_bf16
            .map(|prev| (official_value - prev).abs() == abs_diff)
            .unwrap_or(false)
        || next_bf16
            .map(|next| (next - official_value).abs() == abs_diff)
            .unwrap_or(false);
    ...
    let variant_matches = variants
        ...
        .collect::<Vec<_>>();
    let non_current_variant_match = variant_matches
        .iter()
        .copied()
        .find(|name| *name != "current_local_policy");
    let alternate_gqa_match = gqa_variants.iter().any(|entry| {
        entry
            .get("matches_official")
            .and_then(Value::as_bool)
            .unwrap_or(false)
            && !entry
                .get("expected_mapping")
                .and_then(Value::as_bool)
                .unwrap_or(false)
    });

    let metric_mismatches = |pointer: &str| {
        prior_status
            .pointer(pointer)
            .and_then(Value::as_u64)
            .unwrap_or(u64::MAX)
    };
    let masked_logits_mismatch =
        metric_mismatches("/attention_metrics/masked_logits/metrics/mismatches") != 0;
    let attention_probs_exact =
        metric_mismatches("/attention_metrics/attention_probabilities/metrics/mismatches") == 0;
    let weighted_v_exact =
        metric_mismatches("/attention_metrics/weighted_v/metric/metrics/mismatches") == 0;
    let o_proj_exact = metric_mismatches("/attention_metrics/o_proj/metrics/mismatches") == 0;

    let classification = if schema_blocked {
        format!("layer{layer}_raw_qk_single_mismatch_blocked_by_schema")
    } else if alternate_gqa_match {
        format!("layer{layer}_raw_qk_single_mismatch_gqa_mapping_mismatch")
    } else if let Some(name) = non_current_variant_match {
        if name.contains("scale_per_term") {
            format!("layer{layer}_raw_qk_single_mismatch_scale_order_mismatch")
        } else if name.contains("no_output_round") {
            format!("layer{layer}_raw_qk_single_mismatch_output_rounding_mismatch")
        } else {
            format!("layer{layer}_raw_qk_single_mismatch_accumulation_policy_mismatch")
        }
    } else if one_ulp_like && attention_probs_exact && weighted_v_exact && o_proj_exact {
        format!("layer{layer}_raw_qk_single_mismatch_artifact_precision_boundary")
    } else if attention_probs_exact && weighted_v_exact && o_proj_exact {
        format!("layer{layer}_raw_qk_single_mismatch_downstream_nonpropagating")
    } else {
        format!("layer{layer}_raw_qk_single_mismatch_execution_failed")
    };

    let mut top_terms = terms
        .iter()
        .enumerate()
        .map(|(lane, &product)| {
            let q = q_vector.get(lane).copied().unwrap_or(f32::NAN);
            let k = k_vector.get(lane).copied().unwrap_or(f32::NAN);
            (lane, q, k, product)
        })
        .collect::<Vec<_>>();
    top_terms.sort_by(|left, right| right.3.abs().total_cmp(&left.3.abs()));
    let top_terms = top_terms
        .into_iter()
        .take(8)
        .map(|(lane, q, k, product)| {
            json!({
                "lane": lane,
                "q": q,
                "k": k,
                "product": product,
                "scaled_product": product * cli.scale,
            })
        })
        .collect::<Vec<_>>();
        ...
        (0..head_dim.min(8))
            .map(|lane| {
                json!({
                    ...
                    "k": k_vector[lane],
                    "product": terms[lane],
                    "scaled_product": terms[lane] * cli.scale,
                })
            })
            .collect::<Vec<_>>()
    };
    ...
    let status = json!({
        "mode": "layer0_validation_runtime_path",
        "submode": "raw-qk-single-mismatch-debug",
        "classification": classification,
        "runtime_behavior_changed": false,
        "production_routing_changed": false,
        "model_runner_routing_changed": false,
        ...
        "kv_head": kv_head,
        "scale": cli.scale,
        "sink_column": cli.sink_position,
        "sink_handling": {
            "key_column_is_real_token": key_column < cli.token_count,
            "key_column_is_sink": key_column == cli.sink_position,
            "sink_irrelevant_to_this_mismatch": key_column != cli.sink_position,
        },
        "source_status": {
            "attention_bundle_status": attention_status_path.display().to_string(),
        ...
        "source_vectors": {
            "q_post_rope": {
                "artifact": q_post_status,
                "shape": [head_dim],
                "q_head": q_head,
                "finite_summary": if schema_blocked { Value::Null } else { json!(finite_summary(q_vector)) },
                "digest": digest_f32_le(q_vector),
                "prior_validation_metric": prior_status.pointer("/attention_metrics/q_post_rope/metric/metrics").cloned().unwrap_or(Value::Null),
            },
            "grouped_k_post_rope": {
                "artifact": k_post_status,
                "shape": [head_dim],
                "token": key_column,
                "kv_head": kv_head,
                "finite_summary": if schema_blocked { Value::Null } else { json!(finite_summary(k_vector)) },
                "digest": digest_f32_le(k_vector),
                "source": "ordered grouped K post-RoPE all-token history",
            },
            "lane_window_first_8": lane_window,
            "top_abs_product_terms": top_terms,
            "all_source_values_finite": q_vector.iter().all(|value| value.is_finite())
                && k_vector.iter().all(|value| value.is_finite()),
        },
        "dot_variants": variants,
        "gqa_mapping_variants": gqa_variants,
        "official_value": official_value,
        "official_masked_value": official_masked_value,
        "current_local_value": current_local_value,
        "abs_diff": abs_diff,
        "rounding_probe": {
            "official_bf16_value": official_bf16.to_f32(),
            "previous_bf16_value": prev_bf16,
            "next_bf16_value": next_bf16,
            "one_ulp_like": one_ulp_like,
            "variant_matches_official": variant_matches,
        },
        "earliest_mismatch_source": if classification.ends_with("_gqa_mapping_mismatch") {
            "GQA head mapping variant matched official"
        } else if classification.ends_with("_accumulation_policy_mismatch") {
            "raw-QK dot-product accumulation policy"
        } else if classification.ends_with("_scale_order_mismatch") {
            "raw-QK scale placement"
        } else if classification.ends_with("_output_rounding_mismatch") {
            "raw-QK output rounding boundary"
        } else if classification.ends_with("_artifact_precision_boundary") {
            "single BF16-ULP-like raw-QK artifact precision/readout boundary"
        } else {
            "raw-QK single-entry mismatch did not propagate downstream"
        },
        "downstream_propagation": {
            "masked_logits_mismatch": masked_logits_mismatch,
            "masked_logits_value": {
                "local": current_local_value,
                "official": official_masked_value,
                "abs_diff": (current_local_value - official_masked_value).abs(),
            },
            "attention_probs_exact": attention_probs_exact,
            "weighted_v_exact": weighted_v_exact,
            "o_proj_exact": o_proj_exact,
            "attention_probabilities_metric": prior_status.pointer("/attention_metrics/attention_probabilities/metrics").cloned().unwrap_or(Value::Null),
            "weighted_v_metric": prior_status.pointer("/attention_metrics/weighted_v/metric/metrics").cloned().unwrap_or(Value::Null),
            "o_proj_metric": prior_status.pointer("/attention_metrics/o_proj/metrics").cloned().unwrap_or(Value::Null),
        },
        "artifacts": {
            "raw_qk": raw_status,
        ...
        "next_bounded_step": if classification.ends_with("_artifact_precision_boundary")
            || classification.ends_with("_downstream_nonpropagating")
        {
...
#[cfg(feature = "cuda")]
fn compute_attention_oproj_cublas_bf16(
    weighted_v: &[f32],
    weight: &[f32],
    bias: &[f32],
    pedantic: bool,
    ...
    let q_dim = weighted_v.len();
    anyhow::ensure!(
        weight.len() == hidden * q_dim,
        "o-proj weight len {} does not match hidden*q_dim {}",
        weight.len(),
        hidden * q_dim
    );
    let context =
    ...
    let input_gpu = stream
        ...
        .map_err(|err| anyhow::anyhow!("BF16 o-proj input upload failed: {err}"))?;
    let weight_gpu = stream
        .clone_htod(&weight_bf16)
        .map_err(|err| anyhow::anyhow!("BF16 o-proj weight upload failed: {err}"))?;
    let mut output_gpu = stream
        .clone_htod(&bias_bf16)
        .map_err(|err| anyhow::anyhow!("BF16 o-proj bias/output upload failed: {err}"))?;
    ...
    let metadata = if pedantic {
        let restore = blas
            ...
            .map_err(|err| anyhow::anyhow!("BF16 o-proj cuBLAS pedantic GEMM failed: {err}"))?;
        json!({
            "helper": "cublas_bf16_pedantic_or_deterministic",
            "api": "CublasHandle::bf16_gemm_pedantic_into",
            "m": 1,
            "n": hidden,
            "k": q_dim,
            "output_initialized_from_bf16_bias": true,
            "beta": 1.0,
        ...
        .map_err(|err| anyhow::anyhow!("BF16 o-proj cuBLAS tensor-op GEMM failed: {err}"))?;
        json!({
            "helper": "cublas_bf16_tensor_op",
            "api": "CublasHandle::bf16_gemm_into",
            "m": 1,
            "n": hidden,
            "k": q_dim,
            "output_initialized_from_bf16_bias": true,
            "beta": 1.0,
            "math_mode": "default_tensor_op",
        })
    };
    stream
        .synchronize()
        .map_err(|err| anyhow::anyhow!("BF16 o-proj cuBLAS sync failed: {err}"))?;
    let output = stream
        .clone_dtoh(&output_gpu)
        .map_err(|err| anyhow::anyhow!("BF16 o-proj output download failed: {err}"))?
        .iter()
...
fn compute_attention_oproj_cublas_bf16(
    ...
    anyhow::bail!("cuBLAS BF16 o-proj helper requires the cuda feature")
}

#[cfg(feature = "cuda")]
fn compute_attention_oproj_cublaslt_bf16_bias_epilogue(
    weighted_v: &[f32],
    weight: &[f32],
    bias: &[f32],
) -> Result<(Vec<f32>, Value)> {
    ...
    let stream = context
        .new_stream()
        .map_err(|err| anyhow::anyhow!("CUDA stream init failed: {err}"))?;
    let lt = CublasLtOps::new(stream.clone())
        .map_err(|err| anyhow::anyhow!("cuBLASLt handle init failed: {err}"))?;
    let input_bf16 = weighted_v
        .iter()
        .map(|&value| bf16::from_f32(value))
        .collect::<Vec<_>>();
    ...
    let bias_bf16 = bias
        ...
        .map(|&value| bf16::from_f32(value))
        .collect::<Vec<_>>();
    let output_zero = vec![bf16::from_f32(0.0); hidden];
    let input_gpu = stream
        .clone_htod(&input_bf16)
        .map_err(|err| anyhow::anyhow!("BF16 o-proj input upload failed: {err}"))?;
    let weight_gpu = stream
    ...
    let bias_gpu = stream
        .clone_htod(&bias_bf16)
        .map_err(|err| anyhow::anyhow!("BF16 o-proj bias upload failed: {err}"))?;
    let mut output_gpu = stream
        .clone_htod(&output_zero)
        .map_err(|err| anyhow::anyhow!("BF16 o-proj output upload failed: {err}"))?;

    lt.bf16_matmul_bias_epilogue_a_bt(
        1,
        hidden,
        q_dim,
        1.0,
    ...
    .map_err(|err| anyhow::anyhow!("cuBLASLt BF16 fused-bias o-proj failed: {err}"))?;
    stream
        .synchronize()
        .map_err(|err| anyhow::anyhow!("BF16 o-proj cuBLASLt sync failed: {err}"))?;
    let output = stream
        .clone_dtoh(&output_gpu)
        .map_err(|err| anyhow::anyhow!("BF16 o-proj cuBLASLt output download failed: {err}"))?
        .iter()
        .map(|value| value.to_f32())
        .collect::<Vec<_>>();
    let metadata = json!({
        "helper": "cublaslt_bf16_matmul_bias_epilogue",
        "api": "CublasLtOps::bf16_matmul_bias_epilogue_a_bt",
        "m": 1,
        "n": hidden,
        "k": q_dim,
        "input_dtype": "CUDA_R_16BF",
        "weight_dtype": "CUDA_R_16BF",
        "bias_pointer_dtype": "CUDA_R_16BF",
        "output_dtype": "CUDA_R_16BF",
        "compute_type": "CUBLAS_COMPUTE_32F",
        "scale_type": "CUDA_R_32F",
        "epilogue_type": "CUBLASLT_EPILOGUE_BIAS",
        "alpha": 1.0,
        "beta": 0.0,
        "layout": {
            "logical_expression": "row_major_output[1,hidden] = weighted_v[1,q_dim] @ weight[hidden,q_dim]^T + fused_bias[hidden]",
            "cublaslt_mapping": "C_col[hidden,1] = weight_col[q_dim,hidden]^T @ input_col[q_dim,1]",
            "lda": q_dim,
            "ldb": q_dim,
            "ldc": hidden,
            "transa": true,
            ...
            "stride_b": Value::Null,
            "stride_c": Value::Null,
            "stride_bias": Value::Null,
        },
        "algorithm": {
            "selection": "cudarc_cublaslt_heuristic_fastest_available",
            "heuristic_id": Value::Null,
            "note": "cudarc safe Matmul wrapper selects one heuristic but does not expose the algorithm id"
        },
        "determinism": {
            "atomics_mode": "not_explicitly_controlled_by_cudarc_cublaslt_wrapper",
            "nondeterministic_behavior_allowed": false,
        },
...
fn compute_attention_oproj_cublaslt_bf16_bias_epilogue(
    ...
    anyhow::bail!("cuBLASLt BF16 fused-bias o-proj helper requires the cuda feature")
}
