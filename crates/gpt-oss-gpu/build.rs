...
// locate these PTX files via the GPT_OSS_RS_PTX_DIR env var.
//
...
fn compile_kernels(nvcc: &Path, kernel_dir: &Path, out_dir: &Path) {
    ...
    println!("cargo:rustc-env=GPT_OSS_RS_PTX_DIR={}", ptx_dir.display());
}
...
fn main() {
    ...
    // kernels/ is two levels up from crates/gpt-oss-gpu/
