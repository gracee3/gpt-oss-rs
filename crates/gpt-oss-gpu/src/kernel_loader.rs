...
static KERNEL_FUNCTIONS: &[(&str, &[&str])] = &[
    ...
    (
        "rms_norm_f16",
        &["rms_norm_f16_kernel", "rms_norm_f16_bf16_policy_kernel"],
    ),
...
/// API that understands the gpt-oss-rs kernel naming conventions.
...
impl KernelLoader {
    ...
    ///
    /// Falls back to the `GPT_OSS_RS_KERNEL_DIR` environment variable if `kernel_dir`
    ...
    pub fn new(
        ...
        } else if let Ok(env_dir) = std::env::var("GPT_OSS_RS_KERNEL_DIR") {
