...
//!
//! Loads model weights to GPU via gpt-oss-model-loader's gpu_loader, then runs
...
use gpt_oss_kv_cache::CacheEngine;
...
use gpt_oss_sampling::batch::make_rng;
use gpt_oss_sampling::guided::{GuidedDecodingState, VocabTable};
use gpt_oss_sampling::sampler::Sampler;
use gpt_oss_sequence::{SequenceData, SequenceGroupMetadata};
...
impl Fp8KVCache {
    ...
    fn append_quantized(&mut self, layer_idx: usize, k_new: &[f32], v_new: &[f32]) {
        ...
        for t in 0..num_tokens {
            ...
            let (kq, ks) =
                gpt_oss_kv_cache::quantize_heads(k_slice, self.num_kv_heads, self.head_dim);
            let (vq, vs) =
                gpt_oss_kv_cache::quantize_heads(v_slice, self.num_kv_heads, self.head_dim);
    ...
    fn keys_dequantized(&self, layer_idx: usize) -> Vec<f32> {
        ...
        for t in 0..total_tokens {
            ...
            out.extend_from_slice(&gpt_oss_kv_cache::dequantize_heads(
                data,
                scales,
                self.num_kv_heads,
                self.head_dim,
...
impl GpuWorker {
    pub fn new(config: WorkerConfig) -> Result<Self> {
        ...
        if config.dtype.is_auto() {
            let devices = gpt_oss_gpu::device::list_devices();
    ...
    pub fn from_engine_config(
        model_path: &str,
        config: &gpt_oss_config::EngineConfig,
    ) -> Result<Self> {
    ...
    pub fn load_weights(&mut self, model_path: &Path) -> Result<()> {
        ...
        let (all_weights_loaded, all_weight_shapes) =
            gpt_oss_model_loader::gpu_loader::load_weights_to_gpu_with_shapes(
                model_path,
                &self.stream,
            )
        ...
        #[cfg(feature = "cuda")]
        {
            ...
            let raw_u8_map = gpt_oss_model_loader::gpu_loader::load_u8_weights_to_host(model_path)
            ...
            let raw_u8_map = if self.config.tensor_parallel_size > 1 {
                let (sharded_u8_map, updated_u8_shapes) =
                    gpt_oss_model_loader::shard::shard_u8_host_map(
                        raw_u8_map,
                        &raw_weight_shapes,
            ...
            if self.config.dtype.is_half() {
                ...
                let f16_weights_loaded = gpt_oss_model_loader::gpu_loader::load_weights_to_gpu_f16(
                    model_path,
                    &self.stream,
                )
    ...
    pub fn init_cache(&mut self, num_gpu_blocks: usize, num_cpu_blocks: usize) -> Result<()> {
        ...
        #[cfg(feature = "cuda")]
        {
            use gpt_oss_model_loader::gpu_weights::GpuModelWeights as LoaderWeights;
            ...
            let cache = gpt_oss_kv_cache::engine_cuda::CudaCacheEngine::new(
                self.config.num_layers,
                self.config.num_kv_heads,
                self.config.head_dim,
                block_size,
    ...
    fn find_ptx_dir() -> Option<std::path::PathBuf> {
        ...
        bases.extend([
            std::path::PathBuf::from("/root/gpt-oss-rs/target/release"),
            std::path::PathBuf::from("/root/gpt-oss-rs/target/debug"),
    ...
    fn shard_f32_gpu_weights(
        ...
        let (sharded_host, updated_shapes) = gpt_oss_model_loader::shard::shard_host_map(
            host_weights,
            shapes,
            self.config.tensor_parallel_size,
            self.config.rank,
    ...
    fn shard_f16_gpu_weights(
        ...
        let (sharded_host, _) = gpt_oss_model_loader::shard::shard_host_map(
            host_weights,
            shapes,
            self.config.tensor_parallel_size,
            self.config.rank,
...
mod tests {
    ...
    fn worker_config_from_engine_defaults() {
        let engine = gpt_oss_config::EngineConfig::default();
