...
//! parent `mod.rs`. It delegates the unsafe cuBLAS call to `gpt_oss_gpu::cublas_ops`
...
use gpt_oss_gpu::cublas_ops::CublasOps;
#[cfg(feature = "cublaslt")]
use gpt_oss_gpu::cublaslt_ops::{CublasLtOps, CUBLASLT_M_THRESHOLD};
use half::f16;
use std::sync::Arc;
...
impl CudaLinearLayer {
    ...
    pub fn forward_once_f16(
        ...
        k: usize,
        blas: &CublasHandle,
        loader: &gpt_oss_gpu::kernel_loader::KernelLoader,
    ) -> Result<CudaSlice<f32>> {
...
mod tests {
    ...
    // Run with: cargo test -p gpt-oss-model-runner --features cuda
}
