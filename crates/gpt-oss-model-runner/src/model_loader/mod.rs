...
use gpt_oss_core::config::{ModelConfig, ParallelConfig};
use gpt_oss_core::error::{LLMError, Result};
use tracing::info;

use self::gguf::GGUFLoader;
use self::mapper::WeightMapper;
use self::safetensors::SafeTensorsLoader;
use self::shard::ShardedLoader;
use self::weights::{GpuAllocator, ModelWeights, WeightTensor};
...
#[cfg(test)]
mod tests {
    use super::weights::MockGpuAllocator;
    use super::*;
    use std::io::Write;
    ...
    fn load_model_weights_safetensors() {
        ...
        let data = vec![0u8; 16];
        let file_bytes = super::safetensors::build_test_safetensors(&[(
