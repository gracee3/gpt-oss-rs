...
impl StreamedChatChoiceState {
    ...
    fn ingest(
        ...
        for (tool_index, message) in messages
            .iter()
            .filter(|message| message.role == "assistant")
            .filter(|message| message.recipient.is_some())
            .enumerate()
        {
            ...
            let arguments_delta = diff_text(&state.arguments, &message.content);
            if arguments_delta.is_empty() && state.name == name {
                continue;
            }
            let is_new_call = state.arguments.is_empty();
            state.name = name.clone();
            state.arguments = message.content.clone();
...
impl ChatCompletionToolRequest {
    ...
    /// Convert request tools to gpt-oss-tokenizer ToolDefinitions.
    fn to_tool_definitions(&self) -> Vec<gpt_oss_tokenizer::ToolDefinition> {
        self.tools
            .as_ref()
            .map(|tools| {
                tools
...
pub fn augment_messages_with_tools(
    ...
    tools: &[gpt_oss_tokenizer::ToolDefinition],
    style: gpt_oss_tokenizer::ToolPromptStyle,
) -> Vec<ChatMessage> {
    let tool_text = gpt_oss_tokenizer::format_tool_definitions(tools, style);
...
pub fn preferred_tool_prompt_style(model_name: &str) -> gpt_oss_tokenizer::ToolPromptStyle {
            ...
            .map(|co| {
                ...
                if tools_active {
                    ...
                    match parse_result {
                        ...
                        gpt_oss_tokenizer::ToolParseResult::PlainText(text) => ToolChatChoice {
                            index: co.index,
                            message: ToolChatMessage {
                                role: "assistant".to_string(),
                                content: Some(text),
