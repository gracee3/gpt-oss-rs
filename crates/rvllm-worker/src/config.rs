...
impl WorkerConfig {
    ...
    pub fn model_runner_config(&self) -> Result<rvllm_model_runner::ModelRunnerConfig> {
        let tp_dims = self.tensor_parallel_dims()?;
        Ok(rvllm_model_runner::ModelRunnerConfig {
    ...
    pub fn cache_config(&self) -> Result<rvllm_kv_cache::CacheConfig> {
        let tp_dims = self.tensor_parallel_dims()?;
        Ok(rvllm_kv_cache::CacheConfig::new(
            self.num_layers,
