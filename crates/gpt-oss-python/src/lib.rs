...
use gpt_oss_core::prelude::SamplingParams;
use gpt_oss_sampling::sampler::{Sampler as RustSampler, SamplerOutput as RustSamplerOutput};
...
struct PyTokenizer {
    inner: gpt_oss_tokenizer::Tokenizer,
}
...
impl PyTokenizer {
    ...
    fn from_pretrained(model: &str) -> PyResult<Self> {
        let inner = gpt_oss_tokenizer::Tokenizer::from_pretrained(model)
            .map_err(|e| PyRuntimeError::new_err(e.to_string()))?;
        Ok(Self { inner })
    }
...
impl PyEngineConfig {
    fn to_rust(&self) -> gpt_oss_config::EngineConfig {
        gpt_oss_config::EngineConfig::builder()
            .model(
                gpt_oss_config::ModelConfigImpl::builder()
                    .model_path(&self.model)
                    .dtype(
                        self.dtype
                            .parse::<gpt_oss_core::types::Dtype>()
                            .unwrap_or_default(),
                    )
            ...
            .cache(
                gpt_oss_config::CacheConfigImpl::builder()
                    .gpu_memory_utilization(self.gpu_memory_utilization)
                    .build(),
            )
            .parallel(gpt_oss_config::ParallelConfigImpl {
...
#[pymodule]
fn gpt_oss_rs(m: &Bound<'_, PyModule>) -> PyResult<()> {
