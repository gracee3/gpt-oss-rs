...
/// Re-export from gpt_oss_engine::sequence::SequenceGroupMetadata.
pub use gpt_oss_engine::sequence::SequenceGroupMetadata;

/// Executor-level sampler output for a sequence group.
///
/// Kept local because `gpt_oss_model_runner::sampling::SamplerOutput` models a
/// single token, while the executor output bundles multiple token IDs per
/// group.
#[derive(Debug, Clone)]
...
/// Lightweight executor-facing worker config.
#[derive(Debug, Clone)]
...
/// Lightweight executor-facing worker handle.
pub struct Worker {
