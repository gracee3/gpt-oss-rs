...
//!
//! Loads model weights to GPU via the merged model-runner loader, then runs
...
use gpt_oss_model_runner::gpu_runner::{
    ForwardOutput, Layer0ProjectionSnapshot, PrefillActivationTrace,
};
...
impl Fp8KVCache {
    ...
    fn append_quantized(&mut self, layer_idx: usize, k_new: &[f32], v_new: &[f32]) {
        ...
        for t in 0..num_tokens {
            ...
            let (kq, ks) = gpt_oss_model_runner::kv_cache::quantize_heads(
                k_slice,
                self.num_kv_heads,
                self.head_dim,
            );
            let (vq, vs) = gpt_oss_model_runner::kv_cache::quantize_heads(
                v_slice,
                self.num_kv_heads,
                self.head_dim,
            );
    ...
    fn keys_dequantized(&self, layer_idx: usize) -> Vec<f32> {
        ...
        for t in 0..total_tokens {
            ...
            out.extend_from_slice(&gpt_oss_model_runner::kv_cache::dequantize_heads(
                data,
                scales,
                self.num_kv_heads,
                self.head_dim,
...
impl GpuWorker {
    ...
    pub fn load_weights(&mut self, model_path: &Path) -> Result<()> {
        ...
        #[cfg(feature = "cuda")]
        {
            ...
            let raw_u8_map =
                gpt_oss_model_runner::model_loader::gpu_loader::load_u8_weights_to_host(model_path)
            ...
            let raw_u8_map = if self.config.tensor_parallel_size > 1 {
                let (sharded_u8_map, updated_u8_shapes) =
                    gpt_oss_model_runner::model_loader::shard::shard_u8_host_map(
                        raw_u8_map,
                        &raw_weight_shapes,
            ...
            if self.config.dtype.is_half() {
                ...
                let f16_weights_loaded =
                    gpt_oss_model_runner::model_loader::gpu_loader::load_weights_to_gpu_f16(
                        model_path,
                        &self.stream,
                    )
    ...
    pub fn debug_runner_layer0_projection_snapshot(&self) -> Result<Layer0ProjectionSnapshot> {
        let runner = self.gpu_model_runner.as_ref().ok_or_else(|| {
            LLMError::GpuError("GPU model runner not initialized -- build with --features cuda".into())
        })?;
        runner.debug_layer0_projection_snapshot()
    }
    ...
    fn shard_f32_gpu_weights(
        ...
        let (sharded_host, updated_shapes) =
            gpt_oss_model_runner::model_loader::shard::shard_host_map(
                host_weights,
                shapes,
                self.config.tensor_parallel_size,
                self.config.rank,
    ...
    fn shard_f16_gpu_weights(
        ...
        let (sharded_host, _) = gpt_oss_model_runner::model_loader::shard::shard_host_map(
            host_weights,
            shapes,
            self.config.tensor_parallel_size,
            self.config.rank,
...
fn worker_config_from_engine(
    ...
    WorkerConfig {
        ...
        initial_context_length: config.model.max_model_len,
        rms_norm_eps: 1e-5,
...
mod tests {
    ...
    fn worker_config_from_engine_defaults() {
        let engine = gpt_oss_engine::config::EngineConfig::default();
