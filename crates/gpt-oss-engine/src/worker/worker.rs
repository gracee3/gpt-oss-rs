...
use gpt_oss_core::prelude::{BlockId, LLMError, Result, TokenId};
use gpt_oss_gpu::prelude::GpuStream;
use gpt_oss_model_runner::bridge::{GpuAllocator, MockAttentionBackend, ModelWeights};
use gpt_oss_model_runner::kv_cache::CacheEngine;
use gpt_oss_model_runner::ModelRunner;
use tracing::{debug, info};

use super::config::WorkerConfig;
use super::input::{self, SequenceGroupMetadata};
...
impl Worker {
    ...
    pub fn init_model(&mut self, model_weights: ModelWeights) -> Result<()> {
        ...
        let cache = Arc::new(gpt_oss_model_runner::bridge::CacheEngine::new(
    ...
    pub fn init_cache(&mut self, num_gpu_blocks: usize, num_cpu_blocks: usize) -> Result<()> {
        ...
        #[cfg(feature = "cuda")]
        let kv_alloc = gpt_oss_gpu::cuda_allocator::CudaGpuAllocator::new(self.device_id)?;
        #[cfg(not(feature = "cuda"))]
        let kv_alloc = gpt_oss_gpu::prelude::MockGpuAllocator::new(1 << 30);
    ...
    pub fn warm_up(&self) -> Result<()> {
        ...
        if let Some(runner) = &self.model_runner {
            let dummy_input = gpt_oss_model_runner::ModelInput {
                token_ids: vec![0],
                position_ids: vec![0],
                attention_metadata: gpt_oss_model_runner::bridge::AttentionMetadata {
                    slot_mapping: vec![0],
                    context_lens: vec![1],
                    query_lens: vec![1],
...
mod tests {
    ...
    fn sample_from_logits_basic() {
        ...
        use gpt_oss_engine::sequence::SequenceData;
        ...
        let groups = vec![
            SequenceGroupMetadata {
                ...
                sampling_params: gpt_oss_core::prelude::SamplingParams::default(),
            },
            SequenceGroupMetadata {
