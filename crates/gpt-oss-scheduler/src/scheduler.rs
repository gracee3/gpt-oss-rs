...
use gpt_oss_block_manager::BlockManager;
use gpt_oss_core::prelude::{BlockId, RequestId, Result};
use gpt_oss_sequence::{Sequence, SequenceStatus};

use crate::outputs::{ScheduledSequenceGroup, SchedulerOutputs};
use crate::policy::{PreemptionMode, SchedulerPolicy};

// ---------------------------------------------------------------------------
// SequenceGroup: kept local because gpt_oss_sequence::SequenceGroup has different
// fields (sampling_params, prompt_text) vs our priority/chunked-prefill fields.
// Uses real gpt_oss_sequence::{Sequence, SequenceStatus} for inner sequences.
// TODO: unify with gpt_oss_sequence::SequenceGroup once APIs converge
// ---------------------------------------------------------------------------
...
mod tests {
    use super::*;
    use gpt_oss_block_manager::MemoryPool;
