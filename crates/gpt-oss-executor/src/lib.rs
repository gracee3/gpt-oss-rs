#![forbid(unsafe_code)]
//! Multi-GPU executor orchestration for gpt-oss-rs.
//!
...
// ---------------------------------------------------------------------------
// Wire to real types from gpt-oss-sequence where possible.
// ---------------------------------------------------------------------------

/// Re-export from gpt_oss_sequence::SequenceGroupMetadata.
pub use gpt_oss_sequence::SequenceGroupMetadata;

// ---------------------------------------------------------------------------
// SamplerOutput: kept local because gpt_oss_sampling::SamplerOutput has a
...
// TODO: unify with gpt_oss_sampling::SamplerOutput once APIs converge
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// Worker stubs: kept local because adding gpt-oss-worker as a dependency would
// pull in gpt-oss-model-runner, gpt-oss-kv-cache, gpt-oss-gpu. The executor uses
// these via async channels, so the interface is narrow.
// TODO: wire to gpt_oss_worker::{Worker, WorkerInput, WorkerOutput} once the
// dependency chain is acceptable
// ---------------------------------------------------------------------------

/// Placeholder for `gpt_oss_worker::WorkerConfig`.
#[derive(Debug, Clone)]
...
pub struct WorkerInput {
    ...
    pub blocks_to_swap_in: Vec<(
        gpt_oss_core::prelude::BlockId,
        gpt_oss_core::prelude::BlockId,
    )>,
    pub blocks_to_swap_out: Vec<(
        gpt_oss_core::prelude::BlockId,
        gpt_oss_core::prelude::BlockId,
    )>,
    pub blocks_to_copy: Vec<(
        gpt_oss_core::prelude::BlockId,
        gpt_oss_core::prelude::BlockId,
    )>,
}
...
/// Placeholder for `gpt_oss_worker::Worker`.
pub struct Worker {
...
impl Worker {
    pub fn new(config: WorkerConfig) -> gpt_oss_core::prelude::Result<Self> {
        tracing::info!(
            rank = config.rank,
            gpu = config.gpu_id,
            "worker initialized"
    ...
    pub async fn execute_model(
        &self,
        input: WorkerInput,
    ) -> gpt_oss_core::prelude::Result<WorkerOutput> {
        tracing::debug!(
    ...
    pub async fn check_health(&self) -> gpt_oss_core::prelude::Result<()> {
        Ok(())
    }
