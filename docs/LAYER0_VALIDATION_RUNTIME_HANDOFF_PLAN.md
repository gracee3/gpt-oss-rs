...
validation-only evidence. It is a policy map, not a runtime implementation
plan.
...
The pilot only attempted the first failing seam per layer. Layer7 raw-QK now
has the oracle dtype-probe prerequisite:
