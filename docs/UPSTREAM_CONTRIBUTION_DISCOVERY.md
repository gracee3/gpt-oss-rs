...
| [Intel compute-samples](https://github.com/intel/compute-samples) | Narrow C++ reference used to capture Level Zero capabilities on Tiger Lake | Current revision `b18e178ba784e8eff20c2a57314a6df4d9d2f7c1` fails a GCC 13 syntax compile because `utils.hpp` uses `uint8_t` without directly including `<cstdint>` | OBSERVED | **Low, clear non-Rust exception.** Reproduce with the upstream-supported build before proposing the one-header fix; do not expand this into general C++ maintenance. |
