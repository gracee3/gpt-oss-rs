...
\definecolor{HeaderGray}{gray}{0.25}
...
16\,MB static binary. We describe the modular crate architecture, CUDA integration
...
inference output on NVIDIA A100 at 86~tok/s with an 8\,s startup time, 348\,MB
resident memory, and 6.4\,GB VRAM---compared to Python vLLM~0.18's 200~tok/s with
121\,s startup and 1{,}033\,MB RSS. The throughput gap is primarily due to continuous
batching not yet being enabled. rvLLM is open-source under the Apache-2.0 license.
\end{abstract}
...
\begin{table}[t]
  ...
  \begin{tabular}{@{}ll@{}}
    \toprule
    \rowcolor{HeaderGray} \thead{Architecture} & \thead{Models} \\
    \midrule
...
\begin{table}[t]
  ...
  \begin{tabular}{@{}llp{5.5cm}@{}}
    \toprule
    \rowcolor{HeaderGray} \thead{Kernel} & \thead{File} & \thead{Purpose} \\
    \midrule
...
Table~\ref{tab:gpu_bench} compares rvLLM against Python vLLM~0.18.0 on identical
hardware and model. The throughput gap (0.43$\times$) is expected: rvLLM does not
yet implement continuous batching on the GPU decode path, so each sequence is decoded
individually. Continuous batching and CUDA graph replay are the primary targets for
closing this gap.

\begin{table}[t]
  \centering
  \caption{GPU inference comparison. A100 80\,GB SXM4, Qwen2.5-1.5B, single sequence.}
  \label{tab:gpu_bench}
  \small
  \begin{tabular}{@{}lrrr@{}}
    \toprule
    \rowcolor{HeaderGray} \thead{Metric} & \thead{rvLLM (Rust)} & \thead{Python vLLM 0.18} & \thead{Ratio} \\
    \midrule
    Throughput (tok/s)       & 86            & 200           & 0.43$\times$ \\
    \rowcolor{gray!8}
    Startup time (s)         & 8             & 121           & 15$\times$ faster \\
    Binary / install size    & 16\,MB        & $\sim$500\,MB & 31$\times$ smaller \\
    \rowcolor{gray!8}
    CPU memory RSS           & 348\,MB       & 1{,}033\,MB   & 3.0$\times$ less \\
    GPU VRAM                 & 6.4\,GB       & ---           & --- \\
    \rowcolor{gray!8}
    Output quality           & \multicolumn{2}{c}{Coherent (5/5 prompts)} & --- \\
    \bottomrule
  \end{tabular}
\end{table}

\smallskip\noindent\textit{Note:} The 0.43$\times$ throughput ratio reflects the
absence of continuous batching in rvLLM's current decode path. Each sequence is
decoded individually rather than batched, leaving significant GPU utilisation on the
table. Continuous batching implementation is in progress.
...
\begin{table}[t]
  ...
  \begin{tabular}{@{}lrrrp{3.2cm}@{}}
    \toprule
    \rowcolor{HeaderGray} \thead{Operation} & \thead{Rust} & \thead{Python} & \thead{Speedup} & \thead{Notes} \\
    \midrule
...
\begin{itemize}
  ...
  \item rvLLM does not yet implement the full set of optimisations present in mature
        Python vLLM (e.g., continuous batching on the decode path, CUDA graph replay).
        These account for the majority of the throughput gap.
\end{itemize}
...
infrastructure can yield practical benefits in deployment size (16\,MB binary),
startup time (8\,s vs.\ 121\,s), and memory footprint (348\,MB vs.\ 1{,}033\,MB RSS),
while maintaining API compatibility with the existing vLLM ecosystem. The 22-crate modular architecture
